...
Question set **v2**, model `jev-1.13.0`, gold set **80** repositories, computed 2026-09-20.
...
| **0.5** | 58 | 2 | 12 | 8 | 0.97 | 0.83 | 0.89 |
| 0.6 | 56 | 1 | 14 | 9 | 0.98 | 0.80 | 0.88 |
| 0.7 | 52 | 1 | 18 | 9 | 0.98 | 0.74 | 0.85 |
...
Jev's 0–3 substance score against the human one on 54 repositories: **Spearman 0.75, mean absolute error 0.31**, down from 0.84 / 0.27 before this refresh. Good enough to order projects within a category, which is all the README's top-15 uses it for. Not good enough to read as a grade.
...
| `jkudish/jev-mcp` | 1 | 2.68 | 6.5k README, 10 entries, tests **and** CI |
...
The other two are a disagreement about the rubric, not about the evidence. A repository with a 6.5k README, tests and CI meets the wording of level 2 — "a real tool with several flows, some structure, and tests or examples" — and Jev says 2.68; the human read both as demos and said 1. That is a case for tightening the level descriptions, or for accepting that substance carries about half a point of legitimate spread. It is also a reminder that 54 labels is a small n for a four-level scale.

## 6. Failure cases

Fourteen repositories disagree with the gate at 0.5: twelve misses and, for the first time, two false alarms. The two false alarms are the near-empty repositories dissected in §3 and they are a substance failure, not a gate one. The twelve misses sort by cause:
...
From the 3,683 v2 calls stored in `data/judgments.jsonl`, covering 2,209 repositories:

| | |
|---|---|
| input tokens per call | 3,235 (mean) |
| output tokens per call | ~272 |
| latency p50 / p90 | 210 ms / 337 ms |
| total input tokens | 11.91 M |
| estimated spend | ≈ $0.50 at $0.042 per 1M input tokens (output is free) |

The price is a secondary-source figure. A call costs about $0.00014, so the daily cron — which judges only new or changed repositories — costs well under a cent a day; the $0.50 above is the whole v2 history, re-judges included.
